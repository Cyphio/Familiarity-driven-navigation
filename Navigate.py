...
class Navigate:
    ...
    def database_analysis(self, spacing):
        ...
        for x in probar(x_ticks):
            for y in y_ticks:
                filename = self.grid_data['Filename'].values[(self.grid_data['Grid X'] == x/10) & (self.grid_data['Grid Y'] == y/10)][0]
        ...
        X, Y = np.meshgrid(
            np.arange(self.grid_bounds[0][0], self.grid_bounds[1][0]+1, step=spacing, dtype=int),
            np.arange(self.grid_bounds[0][1], self.grid_bounds[1][1]+1, step=spacing, dtype=int))
