...
class PerfectMemory(AnalysisToolkit):
    ...
    def RIDF(self, curr_view, route_view, route_view_heading=0):
        ...
        for i in np.arange(0, self.vis_deg, step=self.rot_deg, dtype=int):
            rotated_view = np.roll(curr_view, int(curr_view.shape[1] * ((i - route_view_heading) % self.vis_deg / self.vis_deg)), axis=1)
...
if __name__ == "__main__":
    ...
    # pm.view_analysis(curr_view=route_view, save_data=False)